...
export function SidebarPanel({ open, onClose, onSelectSession, onDeleteSession }: SidebarPanelProps) {
  ...
  return (
    <>
      ...
      {/* Body — single vertical scroll containing History (顶部，固定折叠分组) +
          Collections + MCP + Memory 三个共享组件。`space-y-3` 给区域间固定气口；
          `pl-3 pr-0` 保留原本 history 的 12px 左 padding 与 0 右 padding。ScrollArea
          在 4 个区域加起来的总高度超过视口时自动出滚动条——加 MCP / 加 RAG 文件
          都会让列变长，不会被任何固定高度截断。 */}
