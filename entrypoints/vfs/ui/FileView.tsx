 ...
 *  allow-popups"` so the rendered HTML actually runs and link clicks can
 *  open in a new tab.
 *
 ...
 *  set"). `allow-popups` is granted so the `<base target="_blank">` +
 *  override `<script>` injected by the sandbox page (in
 *  `html-preview.sandbox/main.ts`) can actually open a new tab; without
 *  it every link click is silently blocked by Chrome. `allow-top-navigation`
 *  is deliberately excluded so a hostile previewed document can't navigate
 *  the parent sidepanel tab — that would lose the VFS preview the user
 *  is looking at.
 *
